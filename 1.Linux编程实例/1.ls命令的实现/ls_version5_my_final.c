...
void do_ls(char dirname[]){
    ...
    struct dirent **direntp;
	int i = 2;
	int n = scandir(dirname, &direntp, NULL, alphasort);
	int col = get_columns() / 20;
	int lines = ceil(n / col);
	printf("columns = %d\n", col);
	if (n < 0) {
		perror("scandir");
	} else {
...
void do_lsl(char dirname[]){
	...
	if (n < 0) {
		...
		for (int k = 0; k < lines; k++) {
			int cnt = 0;
			while (i < n && cnt < col) {
				print_details(direntp[i]->d_name,dirname);
				free(direntp[i]);
				i++;
				cnt++;
			}
